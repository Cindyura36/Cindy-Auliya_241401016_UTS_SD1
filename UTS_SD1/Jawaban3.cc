...
//Struktur Edge untuk tujuan dan jarak
struct Edge {
    int tujuan;
    int jarak;
};
...
void dfsUtil(int kota, bool visited[], int asal, int jarakDariAsal, int &totalJarak, int &totalLangkah) {
    visited[kota] = true; //Menandai kota yang sudah dikunjungi

    if (asal != -1) { //Jika bukan kota awal
        cout << jarakDariAsal << "m\n"; //Menampilkan jarak dari asal
        totalJarak += jarakDariAsal; //Menambahkan ke total jarak
        totalLangkah++; //Menambahkan ke total langkah
    }
